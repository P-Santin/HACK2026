...
def visualizar_layout_completo(floor_plan, obstacles, bays_catalog, ceiling_profile, bays_colocadas):
    ...
    for bay in bays_colocadas:
        ...
        # GAP
        gap_x = bx - d * math.sin(rad)
        gap_y = by + d * math.cos(rad)
        
        rect_gap = patches.Rectangle((gap_x, gap_y), w, gap, angle=rot,
...
path = "Case0"  
mi_layout = cargar_datos_logistica(str(path))

bays_colocadas = [[0,848.52,8585.78,45]]

print(mi_layout.calcular_porcentaje_ocupacion(bays_colocadas))

visualizar_layout_completo(mi_layout.floor_plan, mi_layout.obstacles, mi_layout.bays, mi_layout.ceiling, bays_colocadas)
